...
function Studio(props) {
  const testUrl = `${process.env.REACT_APP_BACK_ORIGIN}:${process.env.REACT_APP_STREAM_PORT}/live/TESTO.flv`;
  ...
  function Display(event) {
    if (
      event.target.parentNode.parentNode.childNodes[1].attributes[0].value ==
      "false"
    ) {
      event.target.parentNode.parentNode.childNodes[1].attributes[0].value =
        "true";
    ...
    <section>
      ...
      <div aria-haspopup="false" className="containFullMode containVideo">
        ...
        <ReactFlvPlayer
          ...
          isMuted={false}
        />
        <div className="containFullChat">{/* <Chat/> */}</div>
      </div>
    </section>
  );
}
